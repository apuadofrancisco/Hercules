...
static int unit_walktoxy_timer(int tid, int64 tick, int id, intptr_t data)
{
	...
	if (md != NULL && md->walktoxy_fail_count < icewall_walk_block && map->getcell(bl->m, bl, x, y, CELL_CHKICEWALL) && (dx > 0 || dy > 0)) {
	...
	if (sd != NULL) {
		...
		if (map->getcell(bl->m, bl, x, y, CELL_CHKNPC)) {
			...
			nullpo_retr(1, bl->prev); //Script could have warped char, abort remaining of the function.
		} else {
		...
		if (map->getcell(bl->m, bl, x, y, CELL_CHKNPC) && npc->touch_areanpc2(md))
			return 0; // Warped
		else
			md->areanpc_id = 0;
